...
% a superposition of Gauss-Hermite modes, using gh_mode.m.
%
% Arguments:
...
%     + the config fields mentioned in gh_mode.m
%
% Return values:
...
%   >
%   > Et0 = gh_driving_field(0, 0, 0, config);
%   > Et30 = gh_driving_field(0, 0, 30, config);
...
A_nm(1:length(k)) = 0;
A_nm(k>0) = gh_mode(x, y, z, n .* k(k>0), config);
A_nm(k<0) = conj(gh_mode(x, y, z, n .* abs(k(k<0)), config));
